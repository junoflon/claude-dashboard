...
function isAuthenticated(req) {
  if (!DASH_PASSWORD) return true; // no password set = open
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .login { background:#161b22; border:1px solid #30363d; border-radius:12px;
  ...
  .login input {
    width:100%; background:#0d1117; border:1px solid #30363d; color:#f0f6fc;
    padding:10px 14px; border-radius:8px; font-size:14px; outline:none;
    margin-bottom:16px;
  }
  .login input:focus { border-color:#da7756; }
  .login button {
    width:100%; background:#da7756; color:#fff; border:none;
  ...
  try {
    ...
    if (req.url === '/login' && req.method === 'POST') {
      ...
      if (pw === DASH_PASSWORD) {
        ...
        res.writeHead(302, {
          'Set-Cookie': `dash_token=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=604800`,
          'Location': '/'
        });
        res.end();
      } else {
    ...
    if (req.url === '/logout') {
      res.writeHead(302, {
        'Set-Cookie': 'dash_token=; Path=/; HttpOnly; Max-Age=0',
        'Location': '/login'
      });
      res.end();
      return;
    ...
    // ── Sync endpoint (uses its own secret, no session auth) ──
